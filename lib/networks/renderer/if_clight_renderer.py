...
class Renderer:
    ...
    def prepare_sp_input(self, batch):
        ...
        sh = batch["feature"].shape # [bs, 6890, 6]
        sp_input["feature"] = batch["feature"].view(-1, sh[-1])
        # [bsx6890, 6]

        # coordinate: [N, 4], batch_idx, z, y, x
        sh = batch["coord"].shape # bs, 6890, 3 voxel index
        ...
        # concatenate the batch index in front of the voxel coordinates?

        out_sh, _ = torch.max(batch["out_sh"], dim=0)
        # maximum voxel shape with maximum height, width and length
        sp_input["out_sh"] = out_sh.tolist() # three values
        sp_input["batch_size"] = sh[0]       # batchsize 

        sp_input["i"] = batch["i"]           # camera index of each image in the batch
    ...
    def batchify_rays(
        self, sp_input, grid_coords, viewdir, light_pts, chunk=1024 * 32, net_c=None
    ):
    ...
    def render(self, batch):
        ...
        if ray_o.size(1) <= 2048:
            # ray_o should be [bs, N_rays, 3]
            # so if the number of rays is less than 2048, then feed in all the data
            raw = self.net(sp_input, grid_coords, viewdir, light_pts)
        else:
            # else perform batchify_rays
            raw = self.batchify_rays(
                sp_input, grid_coords, viewdir, light_pts, 1024 * 32, None
            )
